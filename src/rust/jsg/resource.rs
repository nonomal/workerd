...
impl<R: Resource> Instance<R> {
        ...
        if state.strong_ref_count == 0 {
            match &mut state.strong_wrapper {
                Some(wrapper) => {
                    ...
                    let isolate = state.isolate.expect("isolate should be set");
    ...
    fn weak_callback(_isolate: *mut v8::ffi::Isolate, data: usize) {
        ...
        // Take the wrapper and forget it - V8 has already reset the weak handle
        // during garbage collection. If we let the Global drop normally, it would
        // try to reset an already-invalid handle, causing a V8 fatal error.
        if let Some(wrapper) = unsafe { (*instance).state.strong_wrapper.take() } {
    ...
    <R as Resource>::Template: 'static,
{
    ...
    match state.strong_wrapper.as_ref().map(|val| val.as_local(lock)) {
        ...
        _ => {
            ...
            unsafe { state.attach_wrapper::<R>(lock.realm(), wrapped_instance.clone().into()) };
            wrapped_instance
        }
    }
}
...
pub fn unwrap<'a, R: Resource>(lock: &'a mut Lock, value: v8::Local<v8::Value>) -> &'a mut R {
    let ptr = unsafe {
        v8::ffi::unwrap_resource(lock.isolate().as_ffi(), value.into_ffi()) as *mut Instance<R>
    };
